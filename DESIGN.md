...
- **公式RSS/API未提供のベンダーソース**: Phase 0 実装時に自動収集不能と判明したため out of scope に移動。
    - Meta AI Blog (RSS未提供、RSSHub にも未解決Issue)
    - Mistral AI News (RSS未提供、Next.js SPA)
    - xAI News (RSS未提供、403ブロック)
    - METR Releases (RSS未提供)
  
  これらはリポジトリ外での解決を想定する (例: 人間が週次で手動確認、RSSHub
  自前ホスティング、ニュースレター購読による間接取得等)。将来これらのサイ
  トが公式RSSを提供した場合は `sources.yaml` に追加して再度 active 化する。
